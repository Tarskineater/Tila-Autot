...
class Accessories
{
	...
	public function ReadAccessory($in_id){
		...
		$output = "";
		$sql = "SELECT accessories.acc_id, accessories.project, accessories.manufactor, accessories.model, " .
		...
		"FROM accessories WHERE accessories.acc_id = '$in_id'";
	...
	public function ReadAccessoryType($acc_id){
		...
		$output = "";
		$sql = "SELECT * FROM `car_acctype` WHERE id = '$acc_id'";
		...
		if ($this->acc_name == "")
			$this->acc_name = "Testaaja Change NAME!";
	...
	/**
	 * Makes Test accessories if there is none
	 */
	public function DoAllAccessories(){
    ...
    public function ShowAccessories($location, $locationname){
		...
		$output = "<table width=\"1050\"><tr><td colspan=\"$l\" class=\"td_phonelistheader\">";	
