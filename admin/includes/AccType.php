...
class AccType
{
	 ...
	 * Read Acctype
	 */
	public function ReadAccType($in_id){
		if (!isset($db))
 			$db = new Database();
		...
		$output = "";
		$sql = "SELECT * FROM `acctype` WHERE id = '$in_id'";
    ...
    public function ShowAccTypes($location, $locationname){
		if (!isset($db))
			$db = new Database();
		...
		if ($page2 == "searchform"){
			...
			$sql = $sql . "cadmin_name FROM acctype";
			$sql = $sql . " ORDER BY id";
		}
		...
		$tmp_acctype="<a href=\"index.php?page=acctype&page2=searchform&product=acctype&order_acctype=";
		
		$output2 ="Accessory type list";
	 ...
	 * Edit Accessory type
	 */
	public function EditAccType($in_id, $location2, $thing){				
		if (!isset($user))
			$user = new User();
		...
		if (isset($_GET["ed"])=="1"){
			...
			$this->ReadAccType($in_id);
		}
		
		$tmp = $this->ReadAccType($this->type);
	...
	/**
	 * Add new accessory type to database
	 */
	public function AddAccType(){
		 ...
		 */
		$sql = "INSERT INTO `acctype` (name, picture, cadmin, changeday, cadmin_name, information)" .
	...
	public function SearchNewst(){
		...
		$output = "";
		$sql = "SELECT MAX(id) FROM `acctype`";
	...
	public function UpdateAccType(){
		 ...
		 * update accessory to database
		 */
		$sql = "UPDATE `acctype` SET name='$this->name', picture='$this->picture', cadmin='$this->cadmin', changeday='$this->changeday', cadmin_name='$this->cadmin_name', information='$this->information' " .
		"WHERE id='$this->id'";
	 ...
	 * Remove acctype
	 */
	public function DeleteAccType($acctype_id){
		if (!isset($db))
			$db = new Database();
		//print "$acctype_id<br>";
		/**
		...
		//print "$output<br>";
 		...
 		if($output=="0"){
			$sql = "DELETE FROM `acctype` WHERE id = '$acctype_id'";
			$db->UseSQL("$sql");
			$output = "0";
		}
	 ...
	 */
	public function DeletedAccType($acctype_id){
		if (!isset($db))
 			$db = new Database();
 		
 		$db->Database();
 		 		
 		$this->ReadAccType($acctype_id);
 		
		/**
		 * Insert Accessory type to database
		 */
		if ($id != ""){	
			$sql = "INSERT INTO `deleted_acctype` (id, name, picture, cadmin, changeday, cadmin_name, information)" .
