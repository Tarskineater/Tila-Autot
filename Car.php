...
class Car{
	...
	public function ShowCars($location, $locationname, $list_type){
		...
		if ($list_type == '0'){
			echo "<table width=\"100%\" height=\"1010\"><tr>";
			echo "<td align=\"center\" class=\"stylish-cBack\"><div id=\"Container\">";
		...
		if (mysql_num_rows($haku) > 0){
			for ($laskuri = 1; $rivi = mysql_fetch_row ($haku); ++$laskuri){
				...
				if ($list_type == '0'){
					...
					if ($rivi[1] != NULL)
						$tmp = "<a href=\"index.php?page=car&id=$rivi[0]&car_id=$rivi[0]&show=1&edtype=$list_type\"><img src=\"images/cars/Car_" . $this->car_plate . "_001.jpg\" alt=\"$rivi[4]\" title=\"$rivi[4]\" width=\"65\" border='0'/></a>";
	...
	*/
	...
	public function CarTypeList(){
		...
		$sql = "SELECT * FROM `types` ORDER BY type_id";
		$haku = $db->AskSQL($sql);
		
		$rows = mysql_num_rows($haku);
		$output = $output . "<div class=\"moduletable\">";
		$output = $output . "<dl class=\"level0\">";
		...
		if ($rows > 0){
			...
			//	if ($ct_id == $rivi[0]){
					...
					$typeName = $this->ReplaceFont($rivi[2]);
					if ($rows2 > 0){
						$output = $output . "<a href=\"index.php?page=car&list=type&car_type=$rivi[0]\" target=\"_top\">";
						if ($rivi[1]==0){
							$output = $output . "<dt class=\"level0 parent\">";
							$output = $output . "<img src=\"images/type/$rivi[4]\" alt=\"Type_$rivi[0]\" title=\"$typeName\" border='0'/>";
							$output = $output . "</dt>\n";
						} else {
							$output = $output . "<dt class=\"level1 notparent\">";
							$output = $output . "$rivi[2]";
							$output = $output . "</dt>\n";
						}
						$output = $output . "</a><br>\n"; 
					} else {
						if ($rivi[1]==0){
							$output = $output . "<dt class=\"level0 parent\">";
							$output = $output . "<img src=\"images/type/$rivi[4]\" alt=\"Type_$rivi[0]\" title=\"$typeName\" border='0'/><br>\n"; 
							$output = $output . "</dt>\n";
						} else {
							$output = $output . "<dt class=\"level1 notparent\">";
							$output = $output . "$rivi[2]";
	...
	public function CarInformation($id){
		...
		echo "<tr><td class=\"stylish-cRows\">Katsastettu</td><td class=\"stylish-cRows\">$this->car_surveyed</td></tr>";
	...
	public function CarPictureBox($id){
		...
		for ($i=1; $i<=15; $i++){
			...
			$tmpimg = "images/cars/Car_" . $this->car_plate . "_" . $this->gNro($i,3) . ".jpg";
			if (file_exists("$tmpimg")){
					//echo $linkki . "$i" . $linkki2 . "<img name='car" . $i . "' src='$tmpimg' onMouseOver=\"document.images['selectedimage'].src = '$tmpimg';\" width='70' height='70' alt='' onClick=\"document.images['selectedimage'].src='$tmpimg'\" ></a>";
					echo "<img name='car" . $i . "' src='$tmpimg' onMouseOver=\"document.images['selectedimage'].src = '$tmpimg';\" width='70' height='70' alt='' onClick=\"document.images['selectedimage'].src='$tmpimg'\" border='0'>";
			}
