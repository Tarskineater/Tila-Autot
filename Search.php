...
class Search{
	...
	public function PrintSQL(){
		...
		print "sql_car='<b>$this->sql_car</b>'<br>";
	...
	public function ProductSelect($name,$product){
		...
		$output1 = $output1 . "<option value =\"\"$l0>&nbsp;- Valitse -&nbsp;</option>\n";  
		$output1 = $output1 . "<option value =\"user\"$l8>&nbsp;K�ytt�j�</option>\n";  
		$output1 = $output1 . "<option value =\"reservation\"$l8>&nbsp;Vuokraus</option>\n";  
		$output1 = $output1 . "<option value =\"car\"$l2>&nbsp;Auto</option>\n";  
		$output1 = $output1 . "<option value =\"acctype\"$l4>&nbsp;Tarvike tyyppi</option>\n";  
		$output1 = $output1 . "<option value =\"accessory\"$l3>&nbsp;Tarvike</option>\n";  
		$output1 = $output1 . "<option value =\"location\"$l6>&nbsp;Paikka</option>\n";  
	...
	public function NewTab(){
		...
		$output = $output . "</td>\n"; 
		$output = $output . "<td class=\"tab\">\n<input type=\"submit\" value=\"Lis��\" id=\"el09\">\n</td>\n";
		$output = $output . "</form>\n";
		$output = $output . "</tr>\n</table>\n";
		/*
		$output = $output . "<table id=\"tabmenu\" cellpadding=\"0\" cellspacing=\"0\"><tr>\n";
		$output = $output . "<form action=\"index.php\" method=\"post\" name=\"list\">\n";
		$output = $output . "<input type=\"hidden\" name=\"page2\" value=\"newform\">\n";
		$output = $output . "<td class=\"tab\" colspan=\"2\">&nbsp;</td>\n";
		$output = $output . "<td class=\"tab\" colspan=\"1\">\n";
		$output = $output . $this->ProductSelect("product","");
		$output = $output . "</td>\n"; 
		$output = $output . "<td class=\"tab\">\n<input type=\"submit\" value=\"Listaa\" id=\"el09\">\n</td>\n";
		$output = $output . "</form>\n";
		$output = $output . "</tr>\n</table>\n";
		*/
		print $output;
	...
	public function TestPageType(){
		...
		*/	
		if((isset($_GET["show"]) && $_GET["show"]=="1") || (isset($_GET["edit"]) && $_GET["edit"]=="1") || (isset($_GET["new"]) && $_GET["new"]=="1") || (isset($_GET["delete"]) && $_GET["delete"]=="1") || (isset($_GET["save"]) && $_GET["save"]=="1"))
			$i=1;
		return $i;
	}
